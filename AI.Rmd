...
This work is composed by five main sections.  The second one deals with the formulation of the research question and will be provided a brief literature review on previous studies. Section three describe the methodology used in order to answer to the research question, how data are collected, cleaned and the techniques used. Section four concern the analysis of the data, it contains the codes and comment about the process and the results. The final section gives a summary of the results, also presenting the main advantages and disadvantages of the techniques and processes used. 
...
From the invention of the World Wide Web until today, this concept has changed and evolved. Among the many implementation that this technology has allowed, the development of social media platform capable of fast and free communication on a global scale has definitely change our lives. It is estimated that, in 2020, the global social penetration reach has reached 49%, with a peak considering Asia and North America that respectively scored a rate of 71% and 69%. Northern Europe fallowed with a slightly lower 67% [^3]. From those statistics is easy to understand the gold mine that social media represent from an opinion mining point of view. 
...
Many studies have been conducted using sentiment analysis and topic modelling on tweets. Indeed, in [^9] S.Yang and H.Zhang has used sentiment analysis and LDA topic modelling in order to understand the potential of these techniques on twitter data. S. Yang et all have proven that LDA allows to easily analyze huge number of tweets and to identify hidden topics. The sentiment analysis was carried on by using the **syuzhet** package and the results has proved that is a powerful tool for understanding and showing the sentiment and emotions present on tweets.  A. Alamsyah [^10] et all, have shown that using LDA and sentiment analysis lead to the capability of extracting meaningful insights about a specific topic using large scale dataset. Compared to traditional method they have stated that those techniques allows better in-time processing.
...
Furthermore, the tweets gathered are only English one with the exclusion of retweet. This because, from one side the sentiment classifier works only with English vocabulary and on the other side because high frequency retweets could distort the percentages of word occurrence. 
...
The next steps will regard Topic Modelling. The aim of this technique is to present the text as a set of topics and so to find different topics among the tweets. Once topics are founded the tweets will be labeled according to that, for further analysis. This procedure will be done by using LDA technique. LDA topic modelling is based on a probabilistic model approach and in particular is based on the assumption that documents are a mixture of topic and that each topic is composed by a certain probability distribution of words.
...
For consistency and reproducibility of the results the following manipulation will be done by loading and using the saved dataset
...
# V. Conclusion, Advantage and Disadvantage

Resuming the research questions:

**There are different conversation topics?**
...
**What people think about this technology, what are the feeling about the topic?**
One pivotal aspect of the analysis was the sentimental one. Categorization that shows an overall positive feeling with the presence of important value for trust and anticipation. Moreover the categorization in the two different topics seems not to cave an influence on the sentiment of tweets. 

**Which one are the words that have the most influence on the sentiment of users?**
